...
// CYD port: which radios actually answered at boot (false = not wired yet).
bool radioPresent[3] = { false, false, false };
...
const int ITEM_HEIGHT = 19;   // enlarged for ~39px finger targets (touch tap-to-select)
const int ITEM_SPACING = 2;
const int TEXT_X = 40;
const int SELECTION_X = 16;
...
void enterMenu(AppMenuState st) {
  ...
  setTouchMode(TOUCH_MENU);   // menu screens: 2-zone BACK/LEVEL bar + tap/drag
  needsRedraw = true;
}
...
void runApp(MenuItem &mi) {
  ...
  setTouchMode(TOUCH_APP);   // apps use the 5-zone arrow D-pad; band gestures off
  mi.setup();
  updateLastActivity();
  displayOff = false;
  u8g2.setPowerSave(0);

  if (!mi.loop) { setTouchMode(TOUCH_MENU); return; }
  ...
  stopAppTracking();
  u8g2.clearBuffer();
  setTouchMode(TOUCH_MENU);   // restore menu tap/drag + bar on app exit
}
...
void setup() {
  ...
  for (int i = 0; i < 3; i++) {
    if (!radios[i].begin() || !radios[i].isChipConnected()) {
      // CYD port: the nRF24 modules may not be wired yet (on order). Do NOT hang
      // the whole device on a missing radio — that left the TFT dark because the
      // u8g2.begin()/display init below never ran. Skip the absent radio and boot;
      // when all three are wired this branch never triggers and behaviour is normal.
      radioPresent[i] = false;
      continue;
    }
    radioPresent[i] = true;
  ...
  // CYD port: the five button pins are now the touch SPI bus / DAC — no pinMode.
  touchInputSetup();
  touchBegin();   // load stored touch calibration, or run 4-corner cal on first boot
...
static int menuStart = 0;   // top visible item index, hoisted from the render for tap math

// Perform the current menu selection (shared by the D-pad SEL press and a touch tap).
...
void loop() {
  ...
  if (justPressed(BUTTON_SEL, selPrev, selDebounceTime)) {
    updateLastActivity();
    if (currentState != APP_LEVEL) activateMenuSelection();
  }
  ...
  if (justPressed(BUTTON_RIGHT, rightPrev, rightDebounceTime)) {
    ...
    if (currentState == APP_MAIN) {
      ...
      levelSystemSetup();
      setTouchMode(TOUCH_APP);   // level screen uses the 5-zone D-pad
      needsRedraw = true;
    }
  }

  // Touch-native menu: tap a row (main area) to enter it; drag the LEFT slider to scroll.
  if (currentState != APP_LEVEL) {
    int sy;
    if (touchMenuSlider(sy)) {                       // left slider = live scroll, never selects
      int sz = getVisibleMenuSize();
      if (sz > 0) {
        int idx = (sy - 100) * sz / 120;
        if (idx < 0) idx = 0;
        if (idx > sz - 1) idx = sz - 1;
        if (idx != item_selected) { item_selected = idx; needsRedraw = true; updateLastActivity(); }
      }
    }
    int ty;
    if (touchMenuTap(ty)) {                          // tap in the main area = select
      int idx = touchScreenYToItem(ty, menuStart);
      if (idx >= 0 && idx < getVisibleMenuSize()) {
        item_selected = idx;
        needsRedraw = true;
        updateLastActivity();
  ...
  if (currentState == APP_LEVEL) {
    ...
    if (needsRedraw) {
      ...
      menuStart = start;   // hoist for tap->item math

      int highlight = item_selected - start;

      int selectionY = 1 + (highlight * (ITEM_HEIGHT + ITEM_SPACING));
      ...
      for (int i = 0; i < 3; i++) {
        ...
        if (idx < getVisibleMenuSize()) {
          ...
          int itemY = 1 + (i * (ITEM_HEIGHT + ITEM_SPACING));
          int textY = itemY + 13;
      ...
      int visSize = getVisibleMenuSize();
      if (visSize > 3) {   // finger-wide scroll slider in the left gutter
        int trackH = 60, thumbH = trackH * 3 / visSize;
        if (thumbH < 10) thumbH = 10;
        int denom = visSize - 1; if (denom < 1) denom = 1;
        int thumbY = 2 + (item_selected * (trackH - thumbH)) / denom;
        u8g2.drawFrame(1, 2, 13, trackH);
        u8g2.drawBox(3, thumbY, 9, thumbH);
      }
